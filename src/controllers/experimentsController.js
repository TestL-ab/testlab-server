...
async function getExperiments(req, res) {
  ...
  try {
    const response = await client.query("SELECT * FROM experiments");
...
async function createExperiment(req, res) {
  const { name, type_id, start_date, end_date, is_running, user_percentage } =
    req.body;
...
async function createVariants(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < variant_arr.length; i++) {
      ...
      if (variant.experiment_id != id)
        throw new Error("Experiment Id doesn't match.");
...
async function createVariant(obj) {
  ...
  try {
    const response = await client.query(
      "INSERT INTO variants (experiment_id, value, is_control, weight) VALUES ($1, $2, $3, $4)",
      [obj.experiment_id, obj.value, obj.is_control, obj.weight]
    );
