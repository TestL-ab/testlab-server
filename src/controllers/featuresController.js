...
async function getFeatures(req, res) {
  ...
  try {
    const response = await client.query("SELECT * FROM features");
...
async function getCurrentFeatures(req, res) {
  ...
  try {
    ...
    let userblocks = await client.query("SELECT * FROM userblocks");
    userblocks = userblocks.rows;
    let currentObj = {
      experiments: currentExperiments,
      toggles: currentToggles,
      rollouts: currentRollouts,
      userblocks,
    };
...
async function updateUserBlocks(currentExperiments) {
  ...
  const client = await pgClient.connect();
  try {
    let response = await client.query("SELECT * FROM userblocks");
...
async function scheduleExperiment(experiment, freeblocks) {
  ...
  try {
    ...
    for (let i = 0; i < freeblocks.length; i++) {
      ...
      let { id } = freeblocks[i];
      await client.query(
        "UPDATE userblocks SET feature_id = $1 WHERE id = $2",
        [feature_id, id]
      );
...
async function resetBlock(block) {
  ...
  try {
    await client.query(
      "UPDATE userblocks SET feature_id = null WHERE name = $1",
      [name]
    );
    return block;
  } catch (error) {
...
async function getVariants(feature_id) {
  ...
  try {
    const response = await client.query(
      "SELECT * FROM variants WHERE feature_id = $1",
      [feature_id]
    );
...
async function createFeature(req, res) {
  ...
  try {
    ...
    response = await client.query("SELECT * FROM features WHERE name = $1", [
      name,
    ]);
...
async function deleteFeature(req, res) {
  ...
  const client = await pgClient.connect();
  try {
    await client.query("DELETE FROM features WHERE (id = $1)", [id]);
...
async function updateFeature(req, res) {
  ...
  try {
    let response = await client.query(
      "UPDATE features SET type_id = $2, name = $3, start_date = $4, end_date = $5, is_running = $6, user_percentage = $7, description = $8 WHERE id = $1",
      [
        id,
        type_id,
        name,
    ...
    response = await client.query("SELECT * FROM features WHERE id = $1", [id]);
    let newFeature = new Feature(response.rows[0]);
    let variants = await getVariants(id);
    if (variants === false) throw new Error("Error getting variants");
    newFeature.variant_arr = variants.map((variant) => {
...
async function createVariants(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < variant_arr.length; i++) {
      ...
      if (variant.feature_id != id)
        throw new Error(
          `feature Id doesn't match. feature id: ${id}. variant id: ${variant.feature_id}`
        );
...
async function createVariant(obj) {
  ...
  const client = await pgClient.connect();
  try {
    let response = await client.query("SELECT * FROM features WHERE id = $1", [
      obj.feature_id,
    ]);
    let features = response.rows;
    if (features.length === 0) throw new Error("No Feature with that name.");
    await client.query(
      "INSERT INTO variants (feature_id, value, is_control, weight) VALUES ($1, $2, $3, $4)",
      [obj.feature_id, obj.value, obj.is_control, obj.weight]
    );
